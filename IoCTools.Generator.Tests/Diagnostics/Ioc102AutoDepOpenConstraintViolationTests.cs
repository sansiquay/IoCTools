...
public sealed class Ioc102AutoDepOpenConstraintViolationTests
{
    ...
    [Fact]
    public void IOC102_fires_when_service_violates_struct_constraint()
    {
        var source = @"
...
";
        ...
        var diag = result.Diagnostics.FirstOrDefault(d => d.Id == "IOC102");
        diag.Should().NotBeNull();
        diag!.GetMessage().Should().Contain("struct");
    }

    [Fact]
    public void IOC102_does_not_fire_when_class_constraint_is_satisfied()
    {
        var source = @"
using IoCTools.Abstractions.Annotations;
...
";
        var result = SourceGeneratorTestHelper.CompileWithGenerator(source, true, OptIn());
        result.Diagnostics.Where(d => d.Id == "IOC102").Should().BeEmpty();
    }

    [Fact]
    public void IOC102_fires_when_interface_constraint_is_not_satisfied()
    {
        var source = @"
using IoCTools.Abstractions.Annotations;
using System;
...
";
    ...
    [Fact]
    public void IOC102_does_not_fire_when_interface_constraint_is_satisfied()
    {
        var source = @"
using IoCTools.Abstractions.Annotations;
using System;
