...
- **`AnalysisScope` model + `DiagnosticGate`** — production-only diagnostics (`IOC081`/`IOC082`/`IOC086`) auto-exempt test projects via the `IsTestProject` MSBuild property forwarded from `Microsoft.NET.Test.Sdk`. No naming heuristics. Test-*support* libraries with no `Microsoft.NET.Test.Sdk` reference (shared fixtures/host-builders) don't get `IsTestProject` automatically — set `<IsTestProject>true</IsTestProject>` in their `.csproj` to opt them into the test scope. See [Project Scope](docs/diagnostics.md#project-scope).
